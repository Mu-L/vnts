...
function validate() {
  if (form.enabled && !form.serverAddress.trim()) { toast.error('启用服务前必须填写服务器地址'); return false }
  if (form.enabled && !form.remoteId.trim()) { toast.error('启用服务前必须填写远程 ID'); return false }
  if (!autoCertificate.value && (!form.cert.trim() || !form.key.trim())) { toast.error('自定义证书模式必须同时填写证书链和私钥路径'); return false }
  return true
}
async function save() {
...
<template>
  <section aria-labelledby="ikev2-title" class="space-y-4">
    <header class="rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:px-6">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="mt-1 text-sm leading-5 text-slate-500 dark:text-slate-400">管理系统原生 VPN 客户端使用的全局连接配置。</p>
          </div>
        </div>
      ...
      <div v-if="saveError || info?.runtime_error" class="mt-4 flex gap-2 rounded-lg border border-red-200 bg-red-50 px-3.5 py-3 text-sm text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300"><AlertCircle :size="18" class="mt-0.5 shrink-0" /><span>{{ saveError || info?.runtime_error }}</span></div>
    </header>
    ...
    <div v-else-if="info" class="space-y-4">
      <section class="settings-card" aria-labelledby="identity-title">
        <div class="section-heading"><div class="section-icon"><Globe2 :size="18" /></div><div><h3 id="identity-title">服务地址与身份</h3><p>客户端连接地址与证书验证身份。</p></div></div>
        <div class="grid gap-5 md:grid-cols-2">
          <div><label for="ikev2-server-address" class="label">服务器地址</label><div class="mt-1.5 flex gap-2"><input id="ikev2-server-address" v-model="form.serverAddress" class="field mt-0" placeholder="vpn.example.com 或 203.0.113.10" :required="form.enabled" /><button type="button" class="secondary-button" @click="useCurrentHost">当前地址</button></div><p class="hint">客户端创建 VPN 连接时填写的域名或 IP 地址。</p></div>
          <div><label for="ikev2-remote-id" class="label">远程 ID</label><input id="ikev2-remote-id" v-model="form.remoteId" class="field" placeholder="vpn.example.com 或 203.0.113.10" :required="form.enabled" /><p class="hint">必须匹配服务器证书中的 SAN。</p></div>
        </div>
      </section>
      ...
      <section class="settings-card" aria-labelledby="client-network-title">
        <div class="section-heading"><div class="section-icon"><Network :size="18" /></div><div><h3 id="client-network-title">客户端网络</h3><p>连接成功后下发给 VPN 客户端的网络参数。</p></div></div>
        <label for="ikev2-dns" class="label">客户端 DNS <span class="font-normal text-slate-400">（可选）</span></label><input id="ikev2-dns" v-model="form.dns" class="field" placeholder="1.1.1.1, 8.8.8.8" /><p class="hint">多个 IPv4 DNS 地址使用逗号或空格分隔。</p>
      </section>

      <section class="settings-card" aria-labelledby="certificate-title">
        <div class="section-heading"><div class="section-icon"><KeyRound :size="18" /></div><div><h3 id="certificate-title">服务器证书</h3><p>用于验证 IKEv2 服务器身份的证书与私钥。</p></div></div>
        ...
        <div v-if="autoCertificate" class="mt-4 rounded-lg border border-cyan-200 bg-cyan-50/70 p-4 dark:border-cyan-500/25 dark:bg-cyan-500/5">
          <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex gap-2.5"><CheckCircle2 :size="18" class="mt-0.5 shrink-0 text-cyan-600 dark:text-cyan-300" /><div><p class="text-sm font-semibold text-slate-800 dark:text-slate-200">证书由服务自动管理</p><p class="mt-1 text-[13px] text-slate-500 dark:text-slate-400">{{ info.certificate_managed ? `当前证书有效期至 ${certificateExpiry}` : '保存后将生成匹配远程 ID 的 RSA-2048 证书' }}</p></div></div>
        ...
        <div v-else class="mt-4 grid gap-5 md:grid-cols-2">
          <div><label for="cert-path" class="label">证书链路径</label><input id="cert-path" v-model="form.cert" class="field" placeholder="/etc/vnts/ikev2-cert.pem" /><p class="hint">PEM 证书链，首张证书的 SAN 必须匹配远程 ID。</p></div>
          <div><label for="key-path" class="label">私钥路径</label><input id="key-path" v-model="form.key" class="field" placeholder="/etc/vnts/ikev2-key.pem" /><p class="hint">匹配证书的 RSA 或 ECDSA P-256 PKCS#8 私钥。</p></div>
          <p v-if="info.certificate_configured && !info.certificate_managed" class="text-[13px] text-slate-500 dark:text-slate-400 md:col-span-2">当前自定义证书有效期至 {{ certificateExpiry }}；私有 CA 需要自行安装到客户端。</p>
        </div>
      </section>
    </div>
